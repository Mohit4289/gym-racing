...
// Start the race with specified duration in milliseconds
function startRace(durationMs) {
    raceStartTime = Date.now();
    const raceEndTime = raceStartTime + durationMs;
    
    // Update race data for selected cycles
    ...
    // Update race stats and check for race end
    function raceLoop() {
        const now = Date.now();
        if (now >= raceEndTime) {
            // Race finished
            ...
            // Race finished, no time display to update
        } else {
            // Update race stats
            updateRaceStats();
            
            // No time display update needed
        }
    }
...
function stopCompetition() {
    ...
    raceData.forEach((data, cycleId) => {
        if (data.selected) {
            ...
            // Update the UI to show 0 energy
            const cycleCard = document.querySelector(`.cycle-card[data-cycle-id="${cycleId}"]`);
            if (cycleCard) {
                const energyDisplay = cycleCard.querySelector('.energy-generated');
                if (energyDisplay) {
                    energyDisplay.textContent = '0 Wh';
                }
            }
        }
    });
...
function updateRaceStats() {
    ...
    document.querySelectorAll('.cycle-card').forEach(card => {
        ...
        if (raceInfo.selected) {
            ...
            raceInfo.energy += (Math.random() * 0.2 + 0.1) * timeDiff; // Random energy generation
